...
/**
 * Interactive exercises catalog. Data is fetched on the server (see page.tsx)
 * and passed in as `initialExercises` — no client loading spinner / waterfall.
 * Search + category/muscle/difficulty filters live here as a Client Component.
 */
export default function ExercisesView({ initialExercises }: { initialExercises: Exercise[] }) {
  const { dict } = useDictionary();
  const t = dict.training.exercises;
  const w = dict.workouts;
  const [allExercises] = useState<Exercise[]>(initialExercises);
